...
def eh(src,obs,val,ch,conf):
    return hashlib.sha256("|".join([src,obs,val,ch,f"{conf:.6f}"]).encode()).hexdigest()
...
class ProvenanceUpdaterTests(unittest.TestCase):
    ...
    def test_green_provenance_builds_candidate_only_update(self):
        with tempfile.TemporaryDirectory() as td:
            ...
            src="https://example.com/source"; obs="2026-09-19T00:00:00+00:00"; val="validator-v1"; ch="abc123"; conf=0.93
            (root/"evidence"/"aion"/"k1.json").write_text(json.dumps({"company_id":"aion","knowledge_id":"k1","source_uri":src,"observed_at":obs,"validated_by":val,"content_hash":ch,"confidence":conf,"evidence_hash":eh(src,obs,val,ch,conf)}),encoding="utf-8")
    ...
    def test_policy_never_becomes_auto_update_candidate(self):
        with tempfile.TemporaryDirectory() as td:
            ...
            src="x"; obs="2026-09-19"; val="v"; ch="h"; conf=1.0
            (root/"evidence"/"aion"/"p1.json").write_text(json.dumps({"company_id":"aion","knowledge_id":"p1","source_uri":src,"observed_at":obs,"validated_by":val,"content_hash":ch,"confidence":conf,"evidence_hash":eh(src,obs,val,ch,conf)}),encoding="utf-8")
            old=os.environ.copy(); self._env(root)
            try: verify_run(); update_run()
            finally: os.environ.clear(); os.environ.update(old)
