...
from jobs.simulate_improvement_candidate_canary import simulate


class CandidateCanarySimulationTests(unittest.TestCase):
    def test_deterministic_canonical_candidate_simulates_green(self):
        with tempfile.TemporaryDirectory() as td:
            ...
            (evidence/"aion.old_vs_new.json").write_text(json.dumps({
                ...
                "comparison_scope":"STRUCTURAL_SPEC_ONLY","live_effect_verified":False,
            ...
            payload=json.loads((evidence/"aion.canary.json").read_text())
            self.assertEqual(payload["status"],"GREEN")
            self.assertEqual(payload["canary_scope"],"LOCAL_SIMULATION_ONLY")
            self.assertFalse(payload["live_traffic_exposed"])
            self.assertFalse(payload["live_effect_verified"])
            self.assertFalse(payload["production_ready"])
            self.assertFalse(payload["external_mutation_allowed"])

    def test_unsupported_candidate_waits(self):
        with tempfile.TemporaryDirectory() as td:
            root=Path(td); candidates=root/"candidates"; evidence=root/"evidence"
            candidates.mkdir(); evidence.mkdir()
            ...
            (evidence/"fenix.old_vs_new.json").write_text(json.dumps({
                "company_id":"fenix","stage":"OLD_VS_NEW","status":"WAITING",
                "comparison_scope":"STRUCTURAL_SPEC_ONLY","live_effect_verified":False,
                "production_ready":False,"external_mutation_allowed":False
            }),encoding="utf-8")
            old=os.environ.copy()
            ...
            payload=json.loads((evidence/"fenix.canary.json").read_text())
            self.assertEqual(payload["status"],"WAITING")

    def test_runtime_advances_through_simulated_canary_then_waits_at_promotion(self):
        with tempfile.TemporaryDirectory() as td:
            ...
            stages=("LAB","TEST","EVALUATE","TRIBUNAL")
            for stage in stages:
            ...
            (evidence/"aion.old_vs_new.json").write_text(json.dumps({
                "company_id":"aion","stage":"OLD_VS_NEW","status":"GREEN","evidence_ref":"e://old-new",
                "live_effect_verified":False,"production_ready":False,"external_mutation_allowed":False
            }),encoding="utf-8")
            (evidence/"aion.canary.json").write_text(json.dumps({
                "company_id":"aion","stage":"CANARY","status":"GREEN","evidence_ref":"e://canary",
