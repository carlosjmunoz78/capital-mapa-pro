...
$StatePath = Join-Path $Runtime "recovery-v1.5.0.json"
$Files = @(
...
$Result = [ordered]@{
  record_type="cerebro_browser_metadata_recovery"; package_version="1.5.0"
...
try {
  ...
  $Backup = Join-Path $Runtime ("recovery-backup-v150-" + (Get-Date -Format "yyyyMMdd-HHmmss"))
  ...
  $Result.next_gate = "RELOAD_EXISTING_CHROME_EXTENSION_V150_AND_REMOTE_LAB_ROUNDTRIP"
  Write-Evidence
  exit 0
} catch {
