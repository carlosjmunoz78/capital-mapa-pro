...
$statePath = Join-Path $runtime 'recovery-v1.4.3.json'
$files = @('CerebroBrowserTransport.ps1')
$result = [ordered]@{record_type='cerebro_browser_transport_recovery';package_version='1.4.3';status='ERROR';stage='START';target='';backup='';checks=[ordered]@{};files=@();launcher=$null;acceptance=$null;error=''}
...
try {
    ...
    $result.checks.zero_mutation_patch = $true
    ...
    $backup = Join-Path $runtime ('recovery-backup-v143-' + (Get-Date -Format 'yyyyMMdd-HHmmss'))
