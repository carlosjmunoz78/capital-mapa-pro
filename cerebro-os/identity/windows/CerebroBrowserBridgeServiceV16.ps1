...
function Get-DefaultState {
    return [ordered]@{
        ...
        lab_operator_selector = ""
        lab_operator_value = ""
        lab_operator_observed_value = ""
    }
}
    ...
    if (Test-Path $StatePath) {
        try {
            ...
            foreach ($key in @(
                ...
                "lab_operator_selector","lab_operator_value","lab_operator_observed_value"
            )) {
...
function Public-State([System.Collections.IDictionary]$State) {
    return [ordered]@{
        ...
        lab_operator_observed_value = [string]$State["lab_operator_observed_value"]
    }
}
    ...
    while ($true) {
        ...
        try {
            ...
            if ($path -eq "/cloud/enqueue") {
                ...
                if ([string]::IsNullOrWhiteSpace($commandId) -or $action -notin @("OPEN_LOCAL_TEST_PAGE","READ_ONLY_PAGE_METADATA","OPERATOR_CLICK","OPERATOR_TYPE","OPERATOR_SELECT","OPERATOR_READ")) {
                ...
                if ($action -like "OPERATOR_*" -and [string]$state["extension_version"] -ne "1.6.0") {
                    Send-Response $stream 400 "application/json; charset=utf-8" '{"status":"BLOCKED","decision":"OPERATOR_V16_EXTENSION_REQUIRED"}'
                    continue
                }
                ...
                $state["lab_operator_observed_value"] = ""
                $state["lab_operator_selector"] = switch ($action) {
                ...
                $state["lab_operator_value"] = switch ($action) {
                    "OPERATOR_TYPE" { "CEREBRO_LAB" }
                    "OPERATOR_SELECT" { "beta" }
                    default { "" }
                }
                Write-State $state
            ...
            if ($path -eq "/lab/operator-fixture") {
                ...
                $body = '<html><head><meta charset="utf-8"><title>CEREBRO Operator LAB Fixture</title></head><body><input id="cerebro-input" maxlength="64"><select id="cerebro-select"><option value="alpha">alpha</option><option value="beta">beta</option></select><button id="cerebro-button" onclick="document.querySelector(&quot;#cerebro-output&quot;).textContent=&quot;CLICKED&quot;">Test</button><output id="cerebro-output">READY</output></body></html>'
            ...
            if ($path -eq "/extension/command") {
                ...
                $payload = [ordered]@{
                    ...
                    selector = [string]$state["lab_operator_selector"]
                    value = [string]$state["lab_operator_value"]
                }
                Send-Response $stream 200 "application/json; charset=utf-8" ($payload | ConvertTo-Json -Compress)
                continue
            }
            if ($path -eq "/extension/result") {
                ...
                $operatorObservedValue = [string]$form["observed_value"]
                ...
                if ($action -like "OPERATOR_*") {
                    $expected = switch ($action) {
                        "OPERATOR_CLICK" { "CLICKED" }
                        "OPERATOR_TYPE" { "CEREBRO_LAB" }
                        "OPERATOR_SELECT" { "beta" }
                        "OPERATOR_READ" { "READY" }
                        default { "" }
                    }
                    $operatorValid = ($result -eq "COMPLETED" -and $expected.Length -gt 0 -and $operatorObservedValue -ceq $expected)
                    if (-not $operatorValid) { $result = "FAILED" }
                    $state["lab_operator_observed_value"] = if ($operatorValid) { $expected } else { "" }
                    $state["lab_command_observed_url"] = ""
                    $state["lab_command_observed_title"] = ""
                    $state["lab_command_page_load_complete"] = $false
                ...
                $state["lab_command_evidence"] = if ($action -like "OPERATOR_*") {
                    if ($result -eq "COMPLETED") { "LAB_OPERATOR_FIXTURE_VERIFIED" } else { "LAB_OPERATOR_FIXTURE_FAILED" }
                } elseif ($action -eq "READ_ONLY_PAGE_METADATA") {
