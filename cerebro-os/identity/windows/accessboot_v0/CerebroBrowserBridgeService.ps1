        ...
        try {
            ...
            if ($path -eq "/cloud/enqueue") {
                ...
                if ($action -eq "BROWSER_OPEN_URL") {
                    $parsed = $null
                    if (-not [Uri]::TryCreate($targetUrl,[UriKind]::Absolute,[ref]$parsed) -or
                        $parsed.Scheme -cne "https" -or [string]::IsNullOrWhiteSpace($parsed.DnsSafeHost) -or
                        $parsed.UserInfo.Length -gt 0 -or $parsed.Port -notin @(-1,443) -or
                        $parsed.DnsSafeHost -in @("localhost","127.0.0.1","::1") -or
                        $parsed.DnsSafeHost -match '^(10\.|192\.168\.|169\.254\.|172\.(1[6-9]|2[0-9]|3[01])\.)') {
                        Send-Response $stream 400 "application/json; charset=utf-8" '{"status":"BLOCKED","decision":"BROWSER_URL_SCOPE_DENIED"}'
                        continue
                    }
                    $targetUrl = $parsed.AbsoluteUri
                }
                ...
                $state["lab_command_value"] = if ($action -in $operatorActions) { $value } elseif ($action -eq "BROWSER_OPEN_URL") { $targetUrl } else { "" }
            ...
            if ($path -eq "/extension/command") {
                ...
                if ($action -eq "READ_ONLY_PAGE_METADATA" -and [string]$state["extension_version"] -notin @("1.5.0","1.6.0","1.6.1","1.6.2")) {
