...
$TransportPatch = "cloud-ack-lease-alignment-p3"
...
function Wait-LocalResult([string]$CommandId) {
    # LOCAL LAB command expires after 180 seconds. Use a monotonic deadline
    # slightly shorter than the Bridge deadline, never a fixed 60-poll budget.
    $deadline = [Diagnostics.Stopwatch]::StartNew()
    try {
        while ($deadline.Elapsed.TotalSeconds -lt 170) {
