...
Deno.serve(async (req: Request) => {
  try {
    ...
    if (req.method === "GET" && path === "/v1/agents/poll") {
      ...
      // Must exceed the transport's 170 s local ACK deadline plus retry margin.
      // The device mutex permits one executor; same-ID replay remains idempotent.
      const leaseUntil = new Date(now.getTime() + 240*1000).toISOString();
