...
# ACCESSBOOT-V0: one-level, policy-bounded file creation under the current
# Windows user's actual Documents known folder (including OneDrive redirection).
# No arbitrary shell, no other paths, no file writes outside the operation ledger.
function Invoke-FsCreateDirectory($Command) {
    $name = [string]$Command.payload.folder_name
    if ($Command.company_id -ne 'fenix' -or $Command.environment -ne 'LAB' -or
        $Command.version -ne 'v0' -or $Command.engine_id -ne 'ACCESSBOOT-001') {
        throw 'FS_SCOPE_DENIED'
    }
    if ($name.Length -lt 1 -or $name.Length -gt 64 -or
        $name -notmatch '^[\p{L}\p{N}][\p{L}\p{N} _-]*
    ...
    if ($action -eq "BROWSER_OPEN_URL") {
        if ([string]$Command.payload.url -cne "https://www.youtube.com/") { throw "BROWSER_URL_POLICY_DENIED" }
        $uri += "&value=" + [uri]::EscapeDataString([string]$Command.payload.url)
    }
    ...
    while ($true) {
        try {
            ...
            if ([string]$cmd.payload.action -eq "FS_CREATE_DIRECTORY") {
                $ok = $ok -and [string]$localResult.evidence_ref -ceq "DOCUMENTS_DIRECTORY_EXISTS_VERIFIED" -and
                    [bool]$localResult.directory_exists -and [string]$localResult.folder_name -ceq [string]$cmd.payload.folder_name
            }
            if ([string]$cmd.payload.action -eq "BROWSER_OPEN_URL") {
                $ok = $ok -and [string]$localResult.evidence_ref -ceq "BROWSER_URL_OPENED" -and
                    [string]$localResult.observed_url -ceq "https://www.youtube.com/" -and [bool]$localResult.page_load_complete
            }
...
try {
    ...
    if ($mutex -and $createdNew) { $mutex.ReleaseMutex() | Out-Null; $mutex.Dispose() }
        ...
        throw 'FS_FOLDER_NAME_DENIED'
    }
    $commandId = [string]$Command.command_id
    if ($commandId -cnotmatch '^[A-Za-z0-9_-]{8,96}
    $action = ([string]$Command.payload.action).ToUpperInvariant()
...
try {
...
) { throw 'FS_COMMAND_ID_DENIED' }
    $documents = [Environment]::GetFolderPath('MyDocuments')
    if (-not $documents -or -not (Test-Path -LiteralPath $documents -PathType Container)) {
        throw 'FS_DOCUMENTS_FOLDER_UNAVAILABLE'
    }
    $target = [IO.Path]::GetFullPath((Join-Path $documents $name))
    if ([IO.Path]::GetDirectoryName($target).TrimEnd('\') -cne ([IO.Path]::GetFullPath($documents)).TrimEnd('\')) {
        throw 'FS_PATH_SCOPE_DENIED'
    }
    $ledgerDir = Join-Path $RuntimeDir 'fs-command-ledger-v0'
    [void][IO.Directory]::CreateDirectory($ledgerDir)
    $ledgerPath = Join-Path $ledgerDir ($commandId + '.json')
    $created = $false
    if (Test-Path -LiteralPath $ledgerPath) {
        $previous = Get-Content -Raw -LiteralPath $ledgerPath -Encoding UTF8 | ConvertFrom-Json
        if ($previous.command_id -cne $commandId -or $previous.company_id -cne 'fenix' -or
            $previous.folder_name -cne $name -or $previous.target -cne $target) {
            throw 'FS_IDEMPOTENCY_CONFLICT'
        }
        if ($previous.phase -notin @('CLAIMED','TERMINAL')) { throw 'FS_LEDGER_CORRUPT' }
    } else {
        $claim = [ordered]@{
            phase='CLAIMED';command_id=$commandId;company_id='fenix';folder_name=$name;target=$target
        }
        $utf8 = New-Object System.Text.UTF8Encoding($false)
        $bytes = $utf8.GetBytes(($claim | ConvertTo-Json -Compress))
        $handle = New-Object System.IO.FileStream($ledgerPath,[IO.FileMode]::CreateNew,[IO.FileAccess]::Write,[IO.FileShare]::None)
        try { $handle.Write($bytes,0,$bytes.Length);$handle.Flush($true) } finally { $handle.Dispose() }
    }
    if (Test-Path -LiteralPath $target) {
        $item = Get-Item -LiteralPath $target -Force
        if (-not $item.PSIsContainer -or ($item.Attributes -band [IO.FileAttributes]::ReparsePoint)) {
            throw 'FS_TARGET_NOT_SAFE_DIRECTORY'
        }
    } else {
    ...
    $finished = [ordered]@{phase='TERMINAL';command_id=$commandId;company_id='fenix';folder_name=$name;target=$target}
    $temp = $ledgerPath + '.tmp'
    $finished | ConvertTo-Json -Compress | Set-Content -LiteralPath $temp -Encoding UTF8
    Move-Item -LiteralPath $temp -Destination $ledgerPath -Force
    return [ordered]@{
        direct=$true
        result=[ordered]@{
            status='COMPLETED';evidence_ref='DOCUMENTS_DIRECTORY_EXISTS_VERIFIED'
            folder_name=$name;folder_path=$target;directory_exists=$true
            local_service_version=$TransportVersion
            external_mutation_performed=$created;secret_value_included=$false
        }
    }
}

function Execute-LocalCommand($Command) {
    if (([string]$Command.payload.action).ToUpperInvariant() -eq 'FS_CREATE_DIRECTORY') {
        return Invoke-FsCreateDirectory $Command
    }
    $action = ([string]$Command.payload.action).ToUpperInvariant()
