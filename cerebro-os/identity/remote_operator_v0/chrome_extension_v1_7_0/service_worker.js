...
async function report(port, extensionId, cmd, success, metadata) {
  ...
  if (cmd.action === "READ_ONLY_PAGE_METADATA" || cmd.action === "BROWSER_OPEN_URL") {
...
async function execute(port, extensionId, cmd) {
  ...
  // Cloud operator V0: independently authorized, exact-target browser navigation.
  // CLAIMED is persisted before opening a tab; an ambiguous crash never opens a second.
  if (cmd.action === "BROWSER_OPEN_URL") {
    const target = "https://www.youtube.com/";
    if (cmd.company_id !== "fenix" || cmd.target_url !== target || cmd.value !== target) return;
    const key = "cerebro_browser_url_receipt_ledger_v170";
    ...
    if (ledger[scope]) {
      if (ledger[scope].phase === "TERMINAL") {
        await report(port,extensionId,cmd,ledger[scope].success === true,ledger[scope].metadata || {});
      } else {
        console.warn("CEREBRO_BROWSER_URL_AMBIGUOUS_NO_REPLAY",cmd.command_id);
      }
      return;
    }
    if (Object.keys(ledger).length >= 128) {
      console.warn("CEREBRO_BROWSER_URL_LEDGER_FULL");
      return;
    }
    ledger[scope]={phase:"CLAIMED",success:false,created_at:Date.now()};
    await chrome.storage.local.set({[key]:ledger});
    let metadata={url:"",title:"",page_load_complete:false};
    let success=false;
    try {
      const tab=await chrome.tabs.create({url:target,active:true});
      if (!tab || !Number.isInteger(tab.id)) throw Error("TAB_NOT_CREATED");
      for (let i=0;i<30;i++) {
        const observed=await chrome.tabs.get(tab.id);
        if (observed.status === "complete") {
          const observedUrl=String(observed.url || "");
          success=observedUrl === target;
          if (success) metadata={url:observedUrl,title:String(observed.title||"").slice(0,120),page_load_complete:true};
          break;
        }
        await new Promise(resolve=>setTimeout(resolve,500));
      }
    } catch (error) { console.warn("CEREBRO_BROWSER_OPEN_FAILED",String(error?.message||"UNKNOWN").slice(0,80)); }
    ledger[scope]={phase:"TERMINAL",success,created_at:ledger[scope].created_at,metadata};
    await chrome.storage.local.set({[key]:ledger});
    await report(port,extensionId,cmd,success,metadata);
    return;
  }
