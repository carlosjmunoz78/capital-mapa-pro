    ...
    while ($true) {
        ...
        try {
            ...
            if ($path -eq "/cloud/enqueue") {
                ...
                if ([string]::IsNullOrWhiteSpace($commandId) -or $action -notin (@("OPEN_LOCAL_TEST_PAGE","READ_ONLY_PAGE_METADATA","BROWSER_OPEN_URL") + $operatorActions)) {
                ...
                if ($action -eq "BROWSER_OPEN_URL" -and
                    ($value -cne "https://www.youtube.com/" -or [string]$state["extension_version"] -cne "1.7.0")) {
                    Send-Response $stream 400 "application/json; charset=utf-8" '{"status":"BLOCKED","decision":"BROWSER_URL_POLICY_DENIED"}'
                    continue
                }
                ...
                $state["lab_command_value"] = if ($action -in $operatorActions -or $action -eq "BROWSER_OPEN_URL") { $value } else { "" }
            ...
            if ($path -eq "/extension/command") {
                ...
                if ($action -eq "READ_ONLY_PAGE_METADATA" -and [string]$state["extension_version"] -notin @("1.5.0","1.6.0","1.6.1","1.6.2","1.7.0")) {
                ...
                $target = if ($action -in @("OPERATOR_CLICK","OPERATOR_TYPE","OPERATOR_SELECT","OPERATOR_READ")) {
                    ...
                    "https://example.com/"
                } elseif ($action -eq "BROWSER_OPEN_URL") {
                    [string]$state["lab_command_value"]
                } else {
            ...
            if ($path -eq "/extension/result") {
                ...
                if ($action -eq "BROWSER_OPEN_URL") {
                    $readbackValid = ($observedUrl -ceq "https://www.youtube.com/" -and $pageLoadComplete)
                    ...
                    $state["lab_command_observed_title"] = if ($readbackValid) { $observedTitle.Substring(0,[Math]::Min($observedTitle.Length,120)) } else { "" }
                ...
                $state["lab_command_evidence"] = if ($action -eq "BROWSER_OPEN_URL") {
                    if ($result -eq "COMPLETED") { "BROWSER_URL_OPENED" } else { "BROWSER_URL_READBACK_FAILED" }
                } elseif ($action -eq "READ_ONLY_PAGE_METADATA") {
