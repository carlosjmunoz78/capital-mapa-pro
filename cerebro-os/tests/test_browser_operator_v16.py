...
class BrowserOperatorV16Tests(unittest.TestCase):
 def test_permissions_are_local_fixture_only(self):
  ...
  self.assertEqual(m["version"],"1.6.0");self.assertEqual(m["host_permissions"],["http://127.0.0.1/*"])
 ...
 def test_executor_is_deterministic_and_no_arbitrary_origin(self):
  ...
  self.assertIn('c.target_url.startsWith("http://127.0.0.1:")',s)
  self.assertNotIn("chrome.cookies",s);self.assertNotIn("https://",s)
