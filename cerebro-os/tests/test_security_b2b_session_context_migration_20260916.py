...
def test_b2b_migration_removes_direct_authenticated_session_context_rpc():
    source = Path("cerebro-os/security/edge-functions/fenix-b2b-actions/index.ts").read_text()
...
def test_b2b_rollback_snapshot_preserved():
    snapshot = Path("cerebro-os/evidence/security/edge-function-snapshots/fenix-b2b-actions-v9.ts").read_text()
