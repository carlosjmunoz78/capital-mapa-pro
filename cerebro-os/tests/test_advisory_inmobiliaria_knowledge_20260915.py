...
VAL = ROOT / "advisory" / "knowledge_validation_inmobiliaria_20260915.json"
STATUS = ROOT / "advisory" / "knowledge_status_20260915.json"


def test_inmobiliaria_uses_bound_canonical_source():
    data = json.loads(VAL.read_text())
    assert data["domain"] == "INMOBILIARIA"
    assert data["source_sha256"] == "27ed1d1e5336a31d1e3a675f741f9dd7842bb0623a504d06dcb5d7f7b413a68a"
    assert data["live_verification_required_per_case"] is True
    assert data["decision_support_only"] is True


def test_inmobiliaria_official_checks_pass():
    data = json.loads(VAL.read_text())
...
def test_inmobiliaria_green_does_not_promote_capability_or_prod():
    data = json.loads(VAL.read_text())
...
def test_global_knowledge_ledger_is_seven_of_twelve():
    data = json.loads(STATUS.read_text())
    assert data["summary"]["domains"] == 12
    assert data["summary"]["knowledge_green"] == 7
    assert data["summary"]["validation_pending"] == 5
    green = {x["domain"] for x in data["domains"] if x["status"] == "KNOWLEDGE_GREEN"}
    assert "INMOBILIARIA" in green
    assert len(green) == 7
