...
class PromotionReadinessRollupTests(unittest.TestCase):
    def test_current_known_gaps_block_prod_candidate(self):
        ...
        self.assertEqual(
            ...
            {"security_review", "recovery_external_proof", "observability_per_engine"},
        )
        self.assertTrue(result["dependency_live_verified"])
        self.assertFalse(result["security_review_green"])
    ...
    def test_unverified_dependencies_remain_a_blocker(self):
        result = module.assess_rollup(
        ...
        self.assertIn("dependency_live_verification", result["blocking_evidence"])
        self.assertIn("security_review", result["blocking_evidence"])
        self.assertFalse(result["prod_candidate"])

    def test_tiktok_is_parked_not_blocking(self):
        result = module.assess_rollup(
            dependency_live_green=True,
            security_review_green=True,
            recovery_external_green=True,
            observability_green=True,
        )
